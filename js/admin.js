...
function loadEquipmentReadyDefaults_() {
  api.getEquipmentReadyDefaults(currentUser.username)
    .then(function (def) {
      el('inputReachTruckReady').value = def.reachTruck || 0;
      el('inputTanggaReady').value = def.tangga || 0;
    })
    .catch(function () { /* biarkan default 0 kalau gagal, tetap bisa diisi manual */ });
}

function loadAssignableUsers() {
  api.getAssignableUsers(currentUser.username)
    .then(function (list) {
...
function kirimKeServer(transaksiRows, stockRows, selectedUsers, reachTruckReady, tanggaReady) {
  api.importRawData(transaksiRows, stockRows, selectedUsers, currentUser.username, reachTruckReady, tanggaReady)
    .then(function (res) {
      el('btnProses').disabled = false;
      el('btnProses').textContent = 'Proses & Bagi Task';
      if (res.success) {
