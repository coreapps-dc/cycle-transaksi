...
function bukaTaskInvestigasi() {
  ...
  api.getInvestigasiFormData(currentUser.username)
    .then(function (data) {
...
// TIDAK ada request baru dan TIDAK ada aksi bulk yang menulis ke server dari sini.
...
function loadOpenTasks() {
  api.getOpenTasks(currentUser.username)
    .then(function (tasks) {
      ...
      tasks.forEach(function (t) {
        ...
        // Bagian C: checkbox multi-select -- murni UI, tidak memicu request apapun.
        ...
        btn.addEventListener('click', function () {
          ...
          api.updateTaskStatus(t.id, currentUser.username, targetStatus, catatan, kategori, picUsername)
            .then(function (res) {
              if (res.success) {
                if (isFinal) {
                  wrap.remove();
...
function loadTaskLog() {
  api.getTaskLog(200, currentUser.username)
    .then(function (logs) {
