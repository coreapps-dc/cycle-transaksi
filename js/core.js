...
function refreshHomeSummary() {
  ...
  api.getHomeSummary(currentUser.username)
    .then(function (res) {
    ...
    .catch(function () { /* diamkan; kpi tetap tampil '–' */ });
}
...
function refreshPrioritasHariIni() {
  ...
  api.getPrioritasHariIni(currentUser.username)
    .then(function (res) {
...
function refreshBadges() {
  api.getMyPendingCount(currentUser.username)
    .then(function (count) {
  ...
  if (currentUser.role === 'inventory' || currentUser.role === 'admin') {
    api.getPendingValidasiCount(currentUser.username)
      .then(function (count) {
...
function refreshCurrentScreen_(btn, reloadFn) {
  ...
  reloadFn();
  // Request async & tidak selalu punya 1 callback tunggal yang jelas (beberapa
  // loader manggil beberapa request sekaligus) -- jadi tombol dilepas lagi
